...
def sgd(data, mini_batches, epochs, learning_rate, polynomials, init_beta=lambda b: np.random.randn(b), metric=metrics.MSE):
    ...
    #beta = init_beta(int(polynomials*(polynomials + 1)/2))
    beta = init_beta(28)
    step = 0
    ...
    for epoch in range(1, epochs + 1):
        ...
        for mini_batch in range(1, mini_batches + 1):
            ...
            beta = beta - learning_rate(step)*cost_diff
...
if __name__ == '__main__':
    ...
    for i, learning_rate in enumerate(np.logspace(-1, -0.8, 3)):
        beta, errors = sgd(data, 20, epochs, lambda t: learning_rate, 6)
